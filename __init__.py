...
try:
    ...
    _access = import_module(f"{_package}.access")
    import_module(f"{_package}.routes").register_routes(
        PromptServer.instance.routes, _runtime.store, _runtime.engine,
        local_session=_access.loopback_listener(args.listen))
