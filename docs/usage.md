...
- 密钥也可通过供应商设置中的环境变量名称读取，但该变量必须先在本地网络策略的 `allowed_key_env` 中获准。环境变量优先于已存密钥。
...
0.2.2 新增独立的服务器出站白名单，默认拒绝未批准的请求；它覆盖 API、模型发现、轮询、取消和图片下载。升级后需要由管理员批准地址，并清除旧的 HTTP 代理配置。详见[网络策略与升级说明](network-policy.md)。
